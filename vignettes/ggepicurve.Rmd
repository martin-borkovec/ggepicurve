...
```{r, fig.height=7}
dat <- dat0[dat0$date <= as.Date("2011-06-01") & dat0$date > as.Date("2011-05-01"),]
ggepicurve(dat,
           date_unit = "day",
           date = "date",
           # mapping = aes(fill = region),
           date_labels = labels_format("%d"),
           col_par = list(geom = "line"))
  
  p + geom_line(inherit.aes = FALSE,
                mapping = aes(x = .date_unit, y = stat(count), group = .date_unit),
                stat = "count")
